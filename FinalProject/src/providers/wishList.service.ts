...
token;
  ...
  getBookDetails(isbn:string,token):Observable<Data[]>{
      console.log('inside service Wish list');
      console.log('in addtowishlist'+isbn);
     let headers = new Headers({ 'Authorization': 'Bearer ' + token });
      let options = new RequestOptions({ headers: headers });
    return this.http.get("http://172.17.120.74:8000/getisbndetails/"+ isbn,options)
     .map((res:Response)  => res.json());
  }
}